...
public class LocalizationTest extends Localization {
	...
	LocalizationTest(String location_filename, String dissim_filename) {
		loadLocationMatrix(location_filename);
		loadDissimilarityMatrix(dissim_filename);	

		setupLocationTraining(0.2);
		setupDissimilarityTraining(.2);
	}
	...
	double[][] runSphereEmbedding(int min_m, int max_m) {
		...
		SphereMDS bestMDS = SphereMDS.bestMDS(Ftrain, min_m, max_m);
		if (bestMDS == null) {
		...
		System.out.println("..........best_curvature k=" + bestMDS.k + ", distort= " + bestMDS.embedding_distortion);
		
		curvature_k = bestMDS.k;
	...
	double[][] runEuclideanEmbedding(int min_m, int max_m) {
		...
		System.out.println("..........distort= " + bestMDS.train_error);
	...
	void runTestMR() {
		...
		for (int i: dissimilarity_testNodes) {
			...
			for (int j: dissimilarity_testNodes) {
				W[i][j] = (i==j)? 0: 1-SphereMDS.geodesic(X[i], X[j], curvature_k);
			}
		}
		localizationMR(X, W);
		...
		for (int i: dissimilarity_testNodes) {
			...
			for (int j: dissimilarity_testNodes) {
				Z[i][j] = (i==j)? 0: SphereMDS.geodesic(X[i], X[j], curvature_k);
			}
		}
	...
	public static void main(String[] args) {
		...
		LocalizationTest p = new LocalizationTest(location_filename, dissim_filename);
		p.runTestMR();
